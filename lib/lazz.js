...
Lazz = (function() {
  ...
  Lazz.prototype.thatsAll = function() {
    var stepCompile, stepProcess, stepRead, stepWrite;
    ...
    stepCompile = (function(_this) {
      return function(done) {
        var types;
        types = _.map(_this._data.file, function(files) {
          return files;
        });
        return async.each(types, stepWrite, done);
      };
    })(this);
    stepWrite = (function(_this) {
      return function(files, done) {
        return async.each(files, _.bind(_this._write, _this), done);
      };
    })(this);
    return async.series([stepRead, stepProcess, stepCompile], function(error) {
      if (error) {
  ...
  Lazz.prototype._readGlobal = function(pattern, done) {
    ...
    readJSON = (function(_this) {
      return function(file, cb) {
        var error;
        try {
          _this._data.meta[utils.keyname(file)] = fs.readJsonSync(_this.path_to(file));
        } catch (_error) {
          error = _error;
  ...
  Lazz.prototype._write = function(file, done) {
